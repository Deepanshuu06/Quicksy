...
exports.placeOrder = async (req, res, next) => {
  try {
    ...
    if (!paymentMethod || !["COD", "ONLINE"].includes(paymentMethod)) {
      throw new ApiError(
        400,
        "Valid payment method is required to place an order"
      );
    }
    ...
    for (const item of cartItems) {
      const inventory = await Inventory.findOne({
        $and: [{ product: item.product._id }, { store: user?.nearbyStore }],
      });
    ...
    const newOrder = new Order({
      ...
      orderStatus: "pending",
      deliveryAddress: user.addresses.includes(addressId)
        ? addressId
        : addressId,
...
exports.cancelOrder = async(req,res,next)=>{
    try{
        ...
        await order.save();
        const response = new ApiResponse(200,"Order cancelled successfully",order);
        res.status(200).json(response);
